...
int main(int argc, char **argv) {
    ...
    for (std::size_t i = 0; i < views.size(); ++i) {
        ...
        mve::ByteImage::Ptr original = view->get_byte_image("original");
        mve::image::byte_to_float_image(original);

        mve::CameraInfo cam = view->get_camera();
        sfm::CameraPose & pose = viewports[i].pose;

        math::Matrix3f K(0.0), R;
        math::Vec3f t;
        cam.fill_camera_translation(t.begin());
        cam.fill_world_to_cam_rot(R.begin());

        float dim_aspect = static_cast<float>(original->width()) / original->height();
